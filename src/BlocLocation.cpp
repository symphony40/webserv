...
void BlocLocation::addAllowedMethods(std::vector<std::string> &tokens)
{
	...
	for (size_t i = 1; i < tokens.size(); i++){
		...
		if (ConfigParser::isMethodSupported(token) == false)
			Logger::log(Logger::FATAL, "Invalid method: \"%s\" in file: %s:%d", token.c_str(), _filename.c_str(), ConfigParser::countFileLines);
		if (token == "GET")
			met = GET;
		else if (token == "POST")
			met = POST;
		...
		if (std::find(_allowedMethods.begin(), _allowedMethods.end(), met) != _allowedMethods.end())
			Logger::log(Logger::FATAL, "Duplicate method: \"%s\" in file: %s:%d", token.c_str(), _filename.c_str(), ConfigParser::countFileLines);
...
e_boolMod BlocLocation::strToBool(std::string &str)
{
	if (str == "on")
		...
		Logger::log(Logger::FATAL, "Invalid value for autoindex: \"%s\" in file: %s:%d", str.c_str(), _filename.c_str(), ConfigParser::countFileLines);
	return (FALSE);
}
...
void BlocLocation::addCgiExtension(std::vector<std::string> &token)
{
	...
	if (_cgiExtension.find(token[1]) != _cgiExtension.end())
		Logger::log(Logger::FATAL, "Duplicate cgi extension: \"%s\" in file: %s:%d", token[1].c_str(), _filename.c_str(), ConfigParser::countFileLines);
...
void BlocLocation::setRewrite(std::vector<std::string>& tokens)
{
	...
	if (code < 300 || code > 399)
		Logger::log(Logger::FATAL, "Invalid return code: \"%s\" in file: %s:%d", tokens[1].c_str(), _filename.c_str(), ConfigParser::countFileLines);
